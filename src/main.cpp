...
// weights and bias calculated with following script:
...
int main() {
  // Count the true/false positives/negatives for the confusion matrix
  uint32_t TN = 0;
  uint32_t TP = 0;
  uint32_t FN = 0;
  uint32_t FP = 0;
  ...
  for (uint32_t index = 0; index < titanic_data_size; index++) {
	// get single entry from dataset
	const struct titanic_entry entry = titanic_data[index];

	// calcualte dot product of entry and weights
    ...
    // normalize such that 0 <= (dot + bias) <= 1
    ...
    // it will increment the correct TN/TP/FN/FP bucket without branching
